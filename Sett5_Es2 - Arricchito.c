#include <stdio.h>
#define GM 10           //Grandezza Matrice: numero colonne e righe matrice: (GM-1 sarà l'ultima colonna o riga)
...
while (carattere < 'Z') {       //ciclo che si protrae fino a che il carattere non è quello precedente a Z
    ...
    if (mossa == 0 && matrice [r-1][c] == '.' && r > 0) {       //per ogni mossa controllo se la cella di destinazione è libera ed esiste

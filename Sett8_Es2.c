...
/*FUNZIONE CONTROLLO*/
/*RESTITUISCE 1 se è presente , 0 se non è presente la matricola (ins_mat) all'interno del vettore studenti riempito fino a (pien)*/
...
while (ripeti == 's') {     //finchè l'utente digita s alla fine del ciclo esso si ripete e torna al menu principale
    ...
    switch (scelta) {
        ...
        case 1 :                    //caso Aggiunta studente
            ...
            ripetizione = controllo (ins_mat, studenti, pien );  /*USO LA FUNZIONE CONTROLLO : RESTITUISCE 1 se è presente , 0 se non è presente*/
            ...
            if (ripetizione == 0) {       //se la matricola non è presente
                ...
                if (i < MAX_ESAMI){
                        i++;
                    studenti[pien].esami[i].voto = 0;     //pone a 0 il voto successivo se è nel vettore
                }
        ...
        case 3:                    //caso Modifica studente
            ...
            if (ripetizione == 1) {
                for (j = 0, ripetizione = 0; j < pien ; j++ ) { //se matricola è presente
...
int controllo (int ins_mat, struct Informazioni_studente *studenti, int pien) {
    printf("\nControllo...\n");
    int j;
    for (j = 0; j < pien ; j++ ) { //controllo se matricola già presente
        if (studenti[j].numero_matricola == ins_mat) {
            return 1;   //memorizzo il fatto che la matricola è già presente
        }
    }
    return 0;
}
